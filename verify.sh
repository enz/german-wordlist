...
! grep -vi '^[a-zäöüßàáâåçčéèêēëīíïîłñōóõœšūûú]*$' words || fatal "invalid characters in word list"
! grep -P '^\p{Ll}+\p{Lu}' words || fatal "words contain inner uppercase"
! grep -P '^\p{Lu}\p{Lu}' words || fatal "words contains abbreviation"
